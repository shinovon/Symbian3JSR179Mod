...
JNIEXPORT jint JNICALL
Java_com_nokia_mj_impl_location_LocationProviderImpl__1getSatelliteData
(JNIEnv* aJni, jobject, jint aPositionerHandle, jintArray aSatelliteData)
{
    jint satellites[2];
	CPositionerBase* positioner =
	        reinterpret_cast<CPositionerBase *>(aPositionerHandle);

    CPositionerBase::GetSatelliteData(positioner, aJni, satellites);

    aJni->SetIntArrayRegion(aSatelliteData, 0, 2, satellites);
	return KErrNone;
}
